...
// Helper to get key from display name
export const getKeyFromDisplayName = (displayName: string): string => {
  const caseType = getCaseTypeByDisplayName(displayName);
  return caseType?.key || displayName.toLowerCase().replace(/\s+/g, '_');
};
...
// Helper to get state key from name
export const getStateKeyFromName = (name: string): string => {
